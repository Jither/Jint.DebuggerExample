...
public static class StringCropExtensions
{
	...
	/// <summary>
	/// Crops start of string to make it fit a maximum length, including a separator (e.g. ellipsis).
	/// </summary>
	/// <remarks>
	/// Note that the string is not guaranteed to be cropped to exactly max length, if the string includes
	/// surrogate pairs (32-bit code points). In that case, it may be shorter, in order to not split a
	/// pair.
	/// </remarks>
	public static string CropStart(this string str, int maxLength, string ellipsis = "...")
	{
		if (maxLength < ellipsis.Length)
		{
			throw new ArgumentOutOfRangeException(nameof(maxLength), maxLength,
		...
		int start = str.Length - (maxLength - ellipsis.Length);
		if (start >= str.Length)
		{
			return ellipsis;
		}
		if (start > 0 && Char.IsSurrogatePair(str, start - 1))
		{
			start++;
		}

		return string.Concat(ellipsis, str.AsSpan(start));
	}
